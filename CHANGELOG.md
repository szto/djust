...
- **`djust.V004` no longer false-fires on user overrides of framework-invoked lifecycle hooks (#1684).** Hooks the framework calls directly (via `self.X()` / `getattr` / `hasattr`), not the user-event router — `handle_presence_join`, `handle_presence_leave`, `handle_cursor_move`, `handle_tick`, `handle_async_result`, `handle_component_event`, `handle_info`, `on_wizard_complete` — match the V004 "event-handler-like" name regex but must **not** carry `@event_handler`, since the framework (not a client event) invokes them. They're now in the V004 lifecycle allowlist in `python/djust/checks.py`, alongside the existing `handle_params` / `handle_event` / etc. Fixes the `djust-org/djust-start#5` symptom, where a `TicTacToeView.handle_presence_leave` override had to suppress `djust.V004` locally. New regression cases in `TestV004LifecycleMethods` (one per hook; each fails against the pre-fix allowlist and passes after).
